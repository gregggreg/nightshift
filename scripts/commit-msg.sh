...
# Prefer the built binary; fall back to `go run` so the hook works in a fresh
# clone before `make build`.
if [[ -x "$REPO_ROOT/nightshift" ]]; then
  RUNNER=("$REPO_ROOT/nightshift")
elif command -v nightshift >/dev/null 2>&1; then
  RUNNER=(nightshift)
elif command -v go >/dev/null 2>&1; then
  RUNNER=(go run "$REPO_ROOT/cmd/nightshift")
else
  echo "  – skipped (no nightshift binary and no go toolchain)"
  exit 0
fi
