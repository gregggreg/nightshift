...
expect accept "whitespace-only (git aborts)" "   "
...
expect accept "leading digit"           "fix: 2x faster budget lookups"
expect accept "leading acronym"         "feat: HTTP retry support"
expect accept "short acronym"           "feat: URL parsing for provider hosts"
expect accept "mixed-case acronym"      "feat: OAuth token refresh"
expect accept "trailing whitespace"     "feat: add budget calibration   "
...
expect reject "trailing period past space" "feat: add budget calibration.  "
expect reject "sentence case"           "feat: Add budget calibration"
expect reject "over 72 once trimmed"    "$over_subject   "
